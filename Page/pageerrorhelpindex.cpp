#include "pageerrorhelpindex.h"

PageErrorHelpIndex::PageErrorHelpIndex(QWidget *parent) : Page(parent)
{
...
void PageErrorHelpIndex::update()
{
    ...
    for(int i=0; i<4; i++)
    {
        ...
        labelText[i]->setFont(textResource.getFont(PAGE_ERROR_HELP_INDEX,"labelText"));
...
void PageErrorHelpIndex::pageShow()
{
    m_strList = textResource.getText(PAGE_ERROR_HELP_INDEX,"labelText");
    m_nPageIndexMax = (m_strList.count()-1)/4;

    if(instance.getPageNumPrev() == PAGE_ERROR_HELP)
...
void PageErrorHelpIndex::pageHide()
{
    emit signalShowPageNum(PAGE_MENU);
}

void PageErrorHelpIndex::mousePressEvent(QMouseEvent *ev)
{
    m_pressPos = ev->pos();
    m_bIsTouch = true;
    m_bIsDrag = false;
...
void PageErrorHelpIndex::mouseReleaseEvent(QMouseEvent *ev)
{
    if (!m_bIsTouch)
        return;
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            instance.setPageNumPrev(PAGE_ERROR_HELP_INDEX);
...
void PageErrorHelpIndex::mouseMoveEvent(QMouseEvent *ev)
{
    Q_UNUSED(ev);
...
void PageErrorHelpIndex::pagePrev()
{
...
void PageErrorHelpIndex::pageNext()
{
    if(m_nPageIndexMax <= 0)
        return;
