#include "pageerrorhelpre.h"

PageErrorHelpRe::PageErrorHelpRe(QWidget *parent) : Page(parent)
{
...
void PageErrorHelpRe::update()
{
    for(int i=0; i<4; i++)
    {
        int dataIndex = m_nPageIndex * 4 + i;

        labelText[i]->setFont(textResource.getFont(PAGE_ERROR_HELP,"labelTextBold"));
...
void PageErrorHelpRe::pageShow()
{
    m_strList = textResource.getText(PAGE_ERROR_HELP,"labelText");
...
void PageErrorHelpRe::pageHide()
{

}

void PageErrorHelpRe::mousePressEvent(QMouseEvent *ev)
{
    m_pressPos = ev->pos();
    m_bIsTouch = true;
    m_bIsDrag = false;
...
void PageErrorHelpRe::mouseReleaseEvent(QMouseEvent *ev)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            instance.setPageNumPrev(PAGE_ERROR_HELP_RE);
...
void PageErrorHelpRe::mouseMoveEvent(QMouseEvent *ev)
{
    Q_UNUSED(ev);
