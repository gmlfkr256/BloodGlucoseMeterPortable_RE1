#include "pageerrorhelpresponse.h"

PageErrorHelpResponse::PageErrorHelpResponse(QWidget *parent) : Page(parent)
{
    this->setGeometry(parent->geometry());
    init();
}

void PageErrorHelpResponse::init()
{
...
void PageErrorHelpResponse::update()
{
...
void PageErrorHelpResponse::pageShow()
{
    m_nCurrentIndex = 0;
    m_strList.clear();

    labelText->setFont(textResource.getFont(PAGE_ERROR_RESPONSE,"labelText"));
    labelNumCurrent->setFont(textResource.getFont(PAGE_ERROR_RESPONSE,"labelNumCurrent"));
    labelNumAll->setFont(textResource.getFont(PAGE_ERROR_RESPONSE,"labelNumAll"));

    int catIdx = instance.nSelectTextIndex;
    int qIdx = instance.nSelectTextIndexSub;

    QString key = QString("labelTextErrorHelp%1-%2").arg(catIdx).arg(qIdx);
    m_strList = textResource.getText(PAGE_ERROR_RESPONSE, key);
...
void PageErrorHelpResponse::mousePressEvent(QMouseEvent *ev)
{
    m_pressPos = ev->pos();
    m_bIsTouch = true;
    m_bIsDrag = false;
...
void PageErrorHelpResponse::mouseReleaseEvent(QMouseEvent *ev)
{
    if(!m_bIsTouch)
        return;
...
void PageErrorHelpResponse::mouseMoveEvent(QMouseEvent *ev)
{
    Q_UNUSED(ev);
...
void PageErrorHelpResponse::pageNext()
{
...
void PageErrorHelpResponse::pagePrev()
{
