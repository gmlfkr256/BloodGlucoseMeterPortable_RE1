...
PageHelp::PageHelp(QWidget *parent) : Page(parent)
{
...
void PageHelp::init()
{
    ...
    connect(timerPress,&QTimer::timeout, this, &PageHelp::handleTimerPress);

    update();
}

void PageHelp::update()
{
    ...
    labelTextTop->setFont(textResource.getFont(PAGE_HELP,"labelText"));
    comText->setFont(textResource.getFont(PAGE_HELP,"labelTextBold"));
    labelTextDown->setFont(textResource.getFont(PAGE_HELP,"labelText"));
}

void PageHelp::pageShow()
{
    m_nSelectIndex = 0;
    int catIdx = instance.nSelectTextIndex;
    m_strList = textResource.getText(PAGE_HELP, QString("labelTextHelp%1").arg(catIdx));
...
void PageHelp::pageHide()
{
    emit signalShowPageNum(PAGE_HELP_INDEX);
}

void PageHelp::mousePressEvent(QMouseEvent *ev)
{
    /*
    if(instance.touchCheck(customButtonOK->geometry(),ev))
    {
        instance.setPageNumPrev(PAGE_HELP);
        instance.nSelectTextIndex = m_nSelectIndex;
        //emit signalShowPageNum(PAGE_RESPONSE);s
...
void PageHelp::mouseReleaseEvent(QMouseEvent *ev)
{
    Q_UNUSED(ev);
    handleTimerRelease();
}

void PageHelp::updateTextSlots()
{
...
void PageHelp::setIndexPrev()
{
    bIsTop = true;
    handleTimerPress();
}

void PageHelp::setIndexNext()
{
    bIsTop = false;
    handleTimerPress();
}

void PageHelp::handleTimerPress()
{
    if(bIsTop)
    {
        movePrev();
...
void PageHelp::handleTimerRelease()
{
    timerPress->stop();
}
