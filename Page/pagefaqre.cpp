...
PageFaqRe::PageFaqRe(QWidget *parent) : Page(parent)
{
...
void PageFaqRe::update()
{
    for(int i=0; i<4; i++)
    {
        int dataIndex = m_nPageIndex * 4 + i;
        labelText[i]->setFont(textResource.getFont(PAGE_FAQ,"labelTextBold"));
...
void PageFaqRe::pageShow()
{
    m_strList = textResource.getText(PAGE_FAQ,"labelText");
...
void PageFaqRe::pageHide()
{

}

void PageFaqRe::mousePressEvent(QMouseEvent *ev)
{
    m_pressPos = ev->pos();
    m_bIsTouch = true;
    m_bIsDrag = false;
...
void PageFaqRe::mouseReleaseEvent(QMouseEvent *ev)
{
    if (!m_bIsTouch)
        return;
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            instance.setPageNumPrev(PAGE_FAQ_RE);
...
void PageFaqRe::mouseMoveEvent(QMouseEvent *ev)
{
    Q_UNUSED(ev);
