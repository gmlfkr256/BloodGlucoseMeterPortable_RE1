...
void PageCaliSelect::init()
{
    ...
    for(int i=0; i<5; i++)
    {
        ...
        labelSelectTextValue[i]->setGeometry(labelSelectButton[i]->geometry().x(),labelSelectButton[i]->y()+120,labelSelectButton[i]->width(),45);
    }
...
void PageCaliSelect::mousePressEvent(QMouseEvent *ev)
{
    for(int i=0; i<5; i++)
    {
        if(instance.touchCheck(labelSelectButton[i]->geometry(),ev))
        {
            if(i!=0)
            {
                if(!instance.caliUserInfo.val[i-1].valid)
                {
