#include "pagehelpindex.h"

PageHelpIndex::PageHelpIndex(QWidget *parent) : Page(parent)
{
...
void PageHelpIndex::update()
{
    ...
    for(int i=0; i<4; i++)
    {
        ...
        labelText[i]->setFont(textResource.getFont(PAGE_HELP_INDEX,"labelText"));
...
void PageHelpIndex::pageHide()
{
    emit signalShowPageNum(PAGE_MENU);
}

void PageHelpIndex::mousePressEvent(QMouseEvent *ev)
{
    m_pressPos = ev->pos();
    m_bIsTouch = true;
    m_bIsDrag = false;
...
void PageHelpIndex::mouseReleaseEvent(QMouseEvent *ev)
{
    if (!m_bIsTouch)
        return;
...
void PageHelpIndex::mouseMoveEvent(QMouseEvent *ev)
{
    Q_UNUSED(ev);
