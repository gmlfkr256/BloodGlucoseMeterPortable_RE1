...
static int clearUserInfoFile (int user, char *fnameP)
{
	char sname[128], buf[80], *str, passwd[32] = { 0, };
	FILE *fp;
	...
	// get serial number
	sprintf (sname, "%s/%s/%s", l_CfgBasePath, GAPI_SYS_VDATA_BASE_DNAME, \
		GAPI_SYS_SERIAL_NUM_FNAME);

	if ((fp = fopen (sname, "r")) != (FILE *) NULL) {
		fgets (buf, sizeof (buf), fp);
		fclose (fp);
		buf[strlen (buf) - 1] = '\0';	// remove '\n'

		if ((str = strrchr (buf, '-')) != (char *) NULL) {
			snprintf (passwd, sizeof (passwd), "u%d%s", user + 1, str);
		}
	}

	if (strlen (passwd) == 0) {
		snprintf (passwd, sizeof (passwd), "u%d-0000000", user + 1);
	}
 ...
 * RETURNS : None
 *****************************************************************************/
int GuiApi::glucoseInitUserPassword (int user)
{
	char fname[80];
 ...
 * RETURNS : None
 *****************************************************************************/
int GuiApi::glucoseChkUpgradeStorage (int *rEcodeP)
{
	char cmd[128], chkrlt[] = "/tmp/chkrlt";
	FILE *fp;
	int rlt;

	if (l_upgradeStorage == GAPI_ACT_START) {
		gapiDebug("UPGRADE storage has alreday used, so deactivate it...\n");
		massStorageAct (GAPI_ACT_STOP, GAPI_SYS_UPGRADE_DEV_NAME, l_CfgUpgradePath);
		l_upgradeStorage = GAPI_ACT_STOP;
		system ("sleep .5");
	}

	sprintf (cmd, "/home/root/chkimage %s; echo $? > %s", l_CfgUpgradePath, chkrlt);
	system (cmd);

	if ((fp = fopen (chkrlt, "r")) == (FILE *)NULL) {
		gapiError("couldn't open file for upgrade\n");
		return GAPI_FAIL;
	}
	fscanf (fp, "%d", &rlt);
	fclose (fp);
